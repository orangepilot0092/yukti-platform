...
# Cache compiled lead qualifier
_cached_qualifier = None
def get_qualifier():
    global _cached_qualifier
    if _cached_qualifier is None:
        print("🔵 Caching compiled Lead Qualifier...", flush=True)
...
async def process_and_reply(user_phone: str, message_text: str, from_name: str):
    ...
    try:
        ...
        print("🟢 INVOKING LEAD QUALIFIER...", flush=True)
        ...
        async with async_session() as session:
            lead = Lead(
                ...
                property_type=final_state.get("property_type"),
                budget_min=final_state.get("budget_min"),
